...
export function AdminCalendar({ slots, view = "day" }: AdminCalendarProps) {
  ...
  return (
    <>
      ...
      {/* Desktop View: Table */}
      <div className="hidden md:block overflow-x-auto rounded-xl border border-white/5 bg-truf-card p-4">
                ...
                <td className="px-4 py-3 text-right whitespace-nowrap">
                  <div className="flex justify-end gap-2">
                    <button
                      onClick={() => handleEditPrice(slot.id, slot.price_paise)}
                      disabled={loadingId === slot.id || slot.status === 'booked'}
                      className="rounded bg-white/10 px-2 py-1 text-xs font-medium text-white hover:bg-white/20 disabled:opacity-50 transition-colors"
                    >
                      Edit Price
                    </button>
                    <button
